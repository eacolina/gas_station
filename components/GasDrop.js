...
async function startPolling(web3Service) {
    ...
    tokenContract = new web3Service.eth.Contract(TOKEN_ABI, process.env.TOKEN_ADDRESS, {from: account})
    console.log(account)
...
async function getTransferEvents(){
    tokenContract.getPastEvents("Transfer",{fromBlock:lastEventBlock, toBlock:'latest'},TransferCB)
    if(doneFirstRun){
        if(log){
